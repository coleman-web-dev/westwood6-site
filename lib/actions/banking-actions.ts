...
export async function mapBankAccountToGL(
  ...
  try {
    ...
    console.error('mapBankAccountToGL permission error:', msg, { communityId, bankAccountId, glAccountId });
    return { success: false, error: `${msg} [debug: cid=${communityId.slice(0, 8)}]` };
  }
